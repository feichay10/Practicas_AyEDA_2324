 ...
 */
template <typename Key>
void selection(StaticSequence<Key>& sequence, int size, bool trace, int& countComparation) {
  Key temp;
 ...
 */
template <typename Key>
void quickSort(StaticSequence<Key>& sequence, int begin, int end, bool trace, int& countComparation) {
  int i = begin;
  int j = end;
  ...
  while (i <= j) {
    while (sequence[i] < pivot) {
      countComparation++; // Incrementa el contador de comparaciones
      i++;
    }
  ...
  if (begin < j) quickSort(sequence, begin, j, trace, countComparation);
  if (i < end) quickSort(sequence, i, end, trace, countComparation);
}
