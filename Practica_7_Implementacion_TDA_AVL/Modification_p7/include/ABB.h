...
class ABB : public AB<Key> {
 ...
 private:
  ...
  bool searchModification(NodeB<Key>* node, const Key k) const;
};
...
bool ABB<Key>::insert(const Key& k) {
  if(searchModification(AB<Key>::root_, k)) {
    return false;
  }
...
template <class Key>
bool ABB<Key>::searchModification(NodeB<Key>* node, const Key k) const {
  if (node == nullptr) {
    return false;
  }
  ...
  if (k < node->getData()) {
    return searchModification(node->getLeft(), k);
  }
  return searchModification(node->getRight(), k);
}
