...
HashTableParameters checkProgramParameters(int argc, char* argv[], HashTableParameters& parameters) {
  ...
  for (int i = 1; i < argc; i++) {
    if (std::string(argv[i]) == "-ts") {
      ...
      if (i + 1 < argc) {
        if (std::stoi(argv[i + 1]) < 1) {
          throw std::invalid_argument("Invalid number of NIFs. Number of NIFs must be greater than 0.");
          exit(EXIT_FAILURE);
        } else {
          parameters.numNif = std::stoi(argv[i + 1]);
        }
      } else {
        throw std::invalid_argument("Number of NIFs not provided.");
        exit(EXIT_FAILURE);
      }
    }
  }
  ...
  std::cout << PURPLE_BOLD << "|" << RESET << GRAY_BOLD
            ...
            << RESET << std::endl;
  std::cout << PURPLE_BOLD << "|" << RESET << GRAY_BOLD
            << " Number of NIFs: " << BLUE_BOLD
            << parameters.numNif << PURPLE_BOLD
            << std::setw(30 - std::to_string(parameters.numNif).length()) << "|"
            << RESET << std::endl;
...
void menu(HashTableType& hashTable, HashTableParameters& parameters) {
  std::string option;
  int optionMenu;
  while (true) {
    std::cout << GRAY_BOLD << "\n###### Hash Table ######" << RESET
    ...
    std::cout << RED_BOLD << "  [6]." << RESET << BOLD << " Print table"
              << std::endl;
    std::cout << RED_BOLD << "  [7]." << RESET << BOLD << " Add random NIFs" << RESET
              << std::endl;
    std::cout << RED_BOLD << "  [8]." << RESET << BOLD << " Exit" << RESET
              << std::endl;
    ...
    switch (optionMenu) {
      ...
      case 2: {
        ...
        hashTable.print();
        std::cout << "Count exploration execution: " << hashTable.getCountExplored() << std::endl;
        break;
      }
      case 3: {
        std::cout << std::endl;
      ...
      case 7: {
        int count = 0;
        std::cout << std::endl;
        for (int i = 0; i < parameters.numNif; i++) {
          keyType randomNif;
          hashTable.insert(randomNif);
          count++;
        }
        hashTable.print();
        std::cout << "\nAdded " << count << " random NIFs" << std::endl;
        std::cout << "Count exploration execution: " << hashTable.getCountExplored() << std::endl;
        break;
      }
      case 8:
